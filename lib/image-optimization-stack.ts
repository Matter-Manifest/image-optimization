...
var CLOUDFRONT_ORIGIN_SHIELD_REGION = ORIGIN_SHIELD_MAPPING.get(
...
type LambdaEnv = {
  S3_ORIGINAL_IMAGE_BUCKET: string;
  S3_TRANSFORMED_IMAGE_BUCKET?: any;
  TRANSFORMED_IMAGE_CACHE_TTL: string;
  SECRET_KEY: string;
  logTiming: string;
  ENV_NAME: string;
};
...
export class ImageOptimizationStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    if (STORE_TRANSFORMED_IMAGES === 'true') {
      transformedImageBucket = new s3.Bucket(this, 'optimized-assets-prod', {
    ...
    var lambdaEnv: LambdaEnv = {
      S3_ORIGINAL_IMAGE_BUCKET: originalImageBucket.bucketName,
      TRANSFORMED_IMAGE_CACHE_TTL: S3_TRANSFORMED_IMAGE_CACHE_TTL,
      SECRET_KEY,
      logTiming: LOG_TIMING,
      ENV_NAME: process.env.ENV_NAME!,
    };
    if (transformedImageBucket)
      lambdaEnv.S3_TRANSFORMED_IMAGE_BUCKET = transformedImageBucket.bucketName;
    ...
    var lambdaProps = {
      // aws-sdk v3 only available on nodeJS18 and above
    ...
    var imageProcessing = new lambda.Function(this, 'assets-optimization-func', lambdaProps);
    ...
    const imageProcessingHelper = new MyCustomResource(this, 'Assets Optimize Resource', {
    ...
    const imageDelivery = new cloudfront.Distribution(this, 'AssetsDistribution', {
      comment: 'Assets Optimization - assets delivery',
      defaultBehavior: imageDeliveryCacheBehaviorConfig,
    });

    new CfnOutput(this, 'ImageDeliveryDomain', {
      description: 'Domain name of assets delivery',
